...
class WinHook
{
	class Shell
	{
		...
		static Remove(Index) {
			item := WinHook.Shell.Hooks.RemoveAt(Index)
			WinHook.Shell.Events[item.Event] := {}	; delete and rebuild Event list
		...
		static Deregister() {
			...
			WinHook.Shell.Hooks := ""
			WinHook.Shell.Events := ""
		}
		; static Message(Event, Hwnd) {
		static Message(params*) {
			DetectHiddenWindows(True)
			Event := params[1]
			Hwnd := params[2]
			try {
				getEvent := WinHook.Shell.Events[Event]
			} catch {
				try {
					getEvent := WinHook.Shell.Events[0]
				} catch {
					getEvent := false
				}
			}
	...
	class Event {
		static Add(eventMin, eventMax, eventProc, idProcess := 0, WinTitle := "") {
			...
			if !IsObject(eventProc) {
				eventProc := Hook.Func(eventProc) ; TODO
			}
		...
		static Message(event, hwnd, idObject, idChild, dwEventThread, dwmsEventTime) {
			DetectHiddenWindows(True)
			Hook := WinHook.Event.Hooks[hWinEventHook := this] ; this' is hidden param1 because method is called as func
